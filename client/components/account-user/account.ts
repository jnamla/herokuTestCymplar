...
namespace accountSettings {
	...
	function config($stateProvider: any) {
        ...
        .state('main-account.user', {
            ...
            views: {
                'main-account': {
                    templateUrl: 'components/account/user-main.html',
                    controller: 'AccountUserMainController',
                    controllerAs: 'accUsrMainCtrl'
                }
            }
        })
        .state('main-account.user.details', {
            url: '/details',
            views: {
                'account-usr-setting': {
                    templateUrl: 'components/account/account-user.html',
                    controller: 'AccountUserDetailsController',
                    controllerAs: 'accUsrDetCtrl'
                }
            }
        });
	}
    ...
    export class AccountUserMainController {        
        /** @ngInject */
        constructor(private $state: any, $stateParams: any, private toastr: any, private $scope: any, private $uibModal: any,
            private user: any, private AuthToken: any) {
        }
    }
    
    export class AccountUserDetailsController {
        
        private userBeautified: string;
        private modalUserDetailsInstance: any;
        ...
        editDetails(user: any) {
            this.modalUserDetailsInstance = this.$uibModal.open({
                templateUrl: 'components/account/account-user-modal.html',
                controller: 'UserDetailsInstanceCtrl',
                controllerAs: 'mUsrDtlCtrl',
                resolve: {
                    user: function () {
                        return user;
                    }
            ...
            this.modalUserDetailsInstance.result.then((selectedItem: any) => {
                this.user = selectedItem;
            });
        };
    }
    
    export class UserDetailsInstanceCtrl {
        
        private checkingUserName: boolean;
		private availableUserName: boolean;
        private newDetails: any;
        private oldDetails: any;
		
        /** @ngInject */
        constructor(private $state: any, $stateParams: any, private toastr: any, private $scope: any, private $uibModalInstance: any,
            private profile: any, private roles: any, private $SignUpRESTService: any, private $AccountRESTService: any) {
            this.newDetails = ObjectUtil.clone(profile);
            this.oldDetails = profile;
        }
  
        save() {
            const modalInstance = this.$uibModalInstance;
            const theDetailsToSave = this.newDetails;
            this.$AccountRESTService.saveAccountOrganizationMember(theDetailsToSave).then(function(response: any) {
                if (response.success) {
                    modalInstance.close(response.data);
                } else {
                    this.toastr.error(response.msg);
        ...
        userNameChanged() {
			...
			if (this.newDetails.email !== undefined && this.newDetails.email.trim() !== '' && this.newDetails.email.trim() !== this.profile.email) {
				this.$SignUpRESTService.isAccountOrganizatioMemberExisted(this.newDetails.email).then((response: any) => {
    ...
    .controller('AccountUserDetailsController', AccountUserDetailsController)
    .controller('UserDetailsInstanceCtrl', UserDetailsInstanceCtrl);
}
